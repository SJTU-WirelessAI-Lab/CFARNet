...
def generate_trajectory(total_time, dt, theta_min_deg, theta_max_deg, r_min, r_max, 
                        min_speed, max_speed, sector_idx=None, total_sectors=3, 
                        random_flag=0, circle_mode=True, min_angle_diff=10.0): # <--- 修改1: 新增参数
    """
    Generate target movement trajectory
    min_angle_diff: 最小角度间隔 (新增参数)
    """
    ...
    theta_safe_min = -60
    theta_safe_max = 60
    # min_angle_diff = 10  <--- 修改2: 移除原来的硬编码，使用传入的参数
    ...
    if circle_mode:
        ...
        if random_flag == 0 and hasattr(generate_trajectory, 'chosen_angles'):
            ...
            # First call or no angle constraints needed
            random_proportion_for_r_squared = np.random.rand()
            r_squared_in_circle = circle["radius"]**2 * random_proportion_for_r_squared
            r_in_circle = np.sqrt(r_squared_in_circle)
            theta_in_circle = 2 * np.pi * np.random.rand()
        ...
        # Randomly select initial distance
        random_proportion_for_r_squared = np.random.rand()
